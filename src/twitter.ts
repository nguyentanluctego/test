...
export class Twitter extends EventEmitter {
    ...
    constructor(
        ...
        this.page.on('request', req => {
            if (req.url() == 'https://twitter.com/i/api/1.1/onboarding/task.json' && req.method() == 'POST') {
    ...
    async enterPhoneNumber(): Promise<boolean> {
        ...
        const resp = await page.waitForResponse('https://twitter.com/i/api/1.1/onboarding/task.json');
        const body = await resp.json();
        console.log(body);
